...
class RobinsonFouldsWindow():
	...
	def __init__(self, master, settings):
		...
		self.description = Label(self.settings_frame,text="Calculate Robinson-Foulds distance")
		...
		self.tree1_label = Label(self.settings_frame, text="Specify first multi-tree file (-t):")
		self.tree1_label.grid(row=3,column=1, sticky=W, columnspan=2)
		
		self.tree1_button = Button(self.settings_frame, text="Load", command=self.load_tree1)
		self.tree1_button.grid(row=3,column=3, sticky=W)
		
		self.tree1_file_label = Label(self.settings_frame, text="no treefile loaded", wraplength=500)
		self.tree1_file_label.configure(foreground="grey")
		self.tree1_file_label.grid(row=4,column=1, sticky=W)
		
		self.tree2_label = Label(self.settings_frame, text="Specify second multi-tree file (-rf):")
		self.tree2_label.grid(row=5,column=1, sticky=W, columnspan=2)
		
		self.tree2_button = Button(self.settings_frame, text="Load", command=self.load_tree2)
		self.tree2_button.grid(row=5,column=3, sticky=W)
		
		self.tree2_file_label = Label(self.settings_frame, text="no treefile loaded", wraplength=500)
		self.tree2_file_label.configure(foreground="grey")
		self.tree2_file_label.grid(row=6,column=1, sticky=W)
		
		self.rf_all_var = IntVar()
		self.rf_all_var.set(1)
		self.rf_adj_var = IntVar()
		...
		self.all = Checkbutton(self.settings_frame, text="Compute all-to-all RF distances between all trees in first tree set (-rf_all)", variable=self.rf_all_var, command=rf_all_m)
		self.all.grid(row=7,column=1, sticky=W)
		self.adj = Checkbutton(self.settings_frame, text="Compute RF distances between adjacent trees in first tree set (-rf_adj)", variable=self.rf_adj_var, command=rf_adj_m)
		self.adj.grid(row=8,column=1, sticky=W)
		...
		self.apply_button = Button(self.settings_frame, text="Calculate Robinson-Foulds distance", command=calc)
