...
class SettingsWindow():	
	...
	def load_iqtree(self):
		...
		self.iqtree_path_entry.delete(0, END)
		self.iqtree_path_entry.insert(END, filename)
	...
	def load_wd(self):
		...
		self.wd_entry.insert(END, dirname)
	...
	def __init__(self, master, data, file_path):
		...
		self.description = Label(self.settings_frame,text="Some basic IQtree GUI settings:", justify=LEFT)
		...
		self.label= Label(self.settings_frame, text="Path to iqtree:")
		self.label.grid(row=3, column=1)
		self.iqtree_path_entry = Entry(self.settings_frame)
		self.iqtree_path_entry.insert(END, data.iqtree_path)
		self.iqtree_path_entry.grid(row=3, column=2)
		self.change_but = Button(self.settings_frame, text="Change", command=self.load_iqtree)
		self.change_but.grid(row=3, column=3)
		
		self.label_wd = Label(self.settings_frame, text="Working directory:")
		self.label_wd.grid(row=4,column=1, columnspan=2, sticky=W)


		# this is currently not working, iqtree seems to produce output in the alignment directory
		self.wd_entry = Entry(self.settings_frame)
		self.wd_entry.insert(END, data.wd)
		self.change_but_wd = Button(self.settings_frame, text="Change", command=self.load_wd)
		self.wd_entry.grid(row=4,column=2)
		self.change_but_wd.grid(row=4, column=3)

		self.label_analysisname = Label(self.settings_frame, text="Analysis name:")
		self.label_analysisname.grid(row=5,column=1, columnspan=2, sticky=W)
		self.analysisname_entry = Entry(self.settings_frame)
		self.analysisname_entry.insert(END, data.analysisname)
		...
		self.apply_change = Button(self.settings_frame, text="Apply & Close", command=set_config_file)
		self.apply_change.grid(row=10, column=4, sticky=W)
		
		self.cancel_button = Button(self.settings_frame, text="Cancel", command=self.master.destroy)
		self.cancel_button.grid(row=10, column=5, sticky=W)
